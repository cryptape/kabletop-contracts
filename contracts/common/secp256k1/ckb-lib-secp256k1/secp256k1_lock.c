...
void blake2b_256(uint8_t * message, size_t len, uint8_t digest[BLAKE2B_BLOCK_SIZE]) {
  blake2b_state blake2b_ctx;
  blake2b_init(&blake2b_ctx, BLAKE2B_BLOCK_SIZE);
  blake2b_update(&blake2b_ctx, message, len);
  blake2b_final(&blake2b_ctx, digest, BLAKE2B_BLOCK_SIZE);
}
